...
char *form_working_dir(struct User *user) {
  ...
  // + 4 for .txt and /
  if ((size_t)strlen(TEMPLATE_RECORDS_DIR) + (size_t)strlen(user->name) + 4 <
      MAX_SIZE) {
    ...
    (void)fprintf(stderr, "Error: Combined length of TEMPLATE_RECORDS_DIR and "
...
char *search_in_file(const char *path, enum formatType format,
  ...
  if (found) {
    ...
    if (!buffer_copy) {
      (void)fprintf(
          stderr, "There was an error processing your transaction's string\n");
      return NULL;
    }
